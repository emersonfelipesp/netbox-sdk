...
| `openbao.py` | `nbx openbao ...` fixed collection CRUD, named custom actions, and bounded binary Raft snapshot transfer. Every mutation and material-bearing read confirms before client creation; all plugin calls use bounded, nonredirecting, nonreplaying transport. Snapshot downloads use exclusive mode-`0600` creation, and restores require typed reason, confirmation, cluster-ID, and Raft-index metadata. |
