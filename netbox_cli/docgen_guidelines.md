...
- The `--live` flag exists only for local developer testing. CI must always use the default (demo) path.

## Command Display Convention

- Commands are documented **without** the `demo` prefix where possible:
  - Document as `nbx dcim devices list`, not `nbx demo dcim devices list`
  - The underlying execution still uses the demo profile exclusively.
...
- **Default profile**: `https://netbox.example.com` with placeholder tokens
...
nbx docs generate-capture --live # default profile (for local testing only)
```
