...
- `netbox_sdk.exceptions` — shared error types (`RequestError`, `BulkOperationEntryError`, facade errors, `JsonPayloadError`, `PaginationError`). `RequestError` parses NetBox 4.7 bulk-failure bodies (`{"detail": ..., "errors": [{"index": N, "errors": {...}}]}`) onto `.detail` and `.entry_errors`; malformed locators (both, neither, bools, negatives) are dropped; other envelopes keep the historical message string
...
- `netbox_sdk.schema` — OpenAPI loading and indexing; `load_openapi_schema()` / `build_schema_index()` default to the bundled NetBox 4.7 GA schema and accept supported release lines such as `version="4.5"`; `SchemaIndex.filter_params(group, resource)` returns a sorted `list[FilterParam]` of filterable query parameters for the list endpoint (excludes pagination params including `limit`, `offset`, `start`, `format`, plus lookup-suffix variants such as `__ic`, `__n`; puts `q` first); `FilterParam` is a frozen Pydantic model with `.name`, `.label`, `.type` (`string|integer|boolean|enum|array`), `.choices` (non-empty only for enum), and `.description`
...
- `netbox_sdk.django_models.catalog` — **the one** Django model-build catalog service, used by both the SDK fetcher and the TUI. Two stores: a read-only **bundled** set shipped as package data under `netbox_sdk/django_models/model_builds/` and read via `importlib.resources` (so it works from a wheel, a zipimport, or a checkout alike), and a **user-writable** store at `$XDG_DATA_HOME/netbox-sdk/django-model-builds` (override with `NETBOX_SDK_MODEL_BUILDS_DIR`) that downloads and generated builds go to — never `site-packages`, which is often read-only. A downloaded build shadows a bundled one of the same tag. `available_tags()` merges both, deterministically newest-first by **numeric** version (`v4.5.10` above `v4.5.9`), so the SDK and TUI can never disagree. The bundled set retains every previously packaged exact patch build while `model_builds/manifest.json` declares the newest numeric patch used for each supported release line; exact `load_build()` requests therefore remain stable, while line matching selects the newest available patch. `DjangoModelStore.build()` records paths relative to the NetBox checkout root. `scripts/build_model_catalog.py` infers that root for older artifacts, rejects any remaining absolute recorded path, and preserves packaged exact builds unless `--prune` is explicitly supplied. `scripts/refresh_django_model_builds.py` imports a locally downloaded workflow artifact or builds from an exact local tag, rejects a local checkout with tracked or untracked changes, normalizes the full archive, regenerates the bundled catalog, and prints `git status --short` for a reviewed Gitea pull request. `django_models_builds/` at the repository root remains the full archive and generator input; it is **not** shipped
...
- `netbox_sdk.plugins` — immutable official-plugin registry. `netbox-openbao`, `netbox-rpc`, and `netbox-proxbox` are first-class; other plugins remain reachable through runtime discovery when standard REST is sufficient.
- `netbox_sdk.openbao` — fixed netbox-openbao collection/action catalog and catalog-backed client. Keep its independent route oracle aligned with canonical plugin `main`, including optional nested routes. Every resource and action uses bounded, nonredirecting, nonreplaying transport with normal authorization-header precedence and defensive write cache invalidation. Raft snapshot helpers preserve bytes, validate opened file descriptors, require typed restore metadata, and enforce request/response bounds. Server-generated SSH keys remain standard credential creation fields (`generate_ssh_key`, `ssh_key_type`); atomic SSH quick-add uses the stable `credential.quick-add-ssh` route.
- `netbox_sdk.rpc` — fixed nine-collection netbox-rpc contract, including complete standard CRUD and bulk writes for both allowlists, policy-correct CRUD, every custom REST action, and bounded execution polling. Generic discovery cannot infer custom actions.
- RPC contract changes must update the fixed collection/action oracle, exact CRUD/bulk transport assertions for every mutable collection, every semantic method transport assertion, the dedicated English/Portuguese SDK and CLI guides, and both root agent mirrors. Collection `create` accepts object or array payloads and owns bulk creation. Preserve confirmation-before-client behavior, repeated query keys on semantic reads, finite positive polling bounds, and cache-free execution state polling.
