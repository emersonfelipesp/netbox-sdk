...
NetBox `4.7`, `4.6`, `4.5`, `4.4` e `4.3`. A linha 4.7 padrão é gerada a partir do schema oficial GA `v4.7.0`. Não é necessário executar geração de código
localmente. A CI testa o SDK contra `v4.7.0`, `v4.6.6`, `v4.6.3`, `v4.6.2` e `v4.5.10`.

## Type hints na IDE

Os quatro pacotes instalados (`netbox_sdk`, `netbox_cli`, `netbox_tui`,
`netbox_mcp`) trazem
...
[Suporte de IDE](../developer/ide-support.md) para o workspace de contribuidor
...
- `pip install 'netbox-sdk[mcp]'` se você quer o servidor `nbx-mcp` por stdio ou
  Streamable HTTP
