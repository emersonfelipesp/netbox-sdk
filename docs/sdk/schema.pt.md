...
O SDK inclui esquemas OpenAPI integrados do NetBox em `netbox_sdk/reference/openapi/` para as linhas de release suportadas. `build_schema_index()` usa por padrão o schema oficial GA do NetBox 4.7, enquanto `version="4.5"` seleciona o esquema 4.5. `SchemaIndex` analisa um esquema uma vez e expõe auxiliares rápidos de consulta para grupos, recursos, caminhos e parâmetros de filtro.
...
# Usa o schema integrado GA do NetBox 4.7 (padrão)
