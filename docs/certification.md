...
| Offline validation | Mock NetBox API, typed-client tests, schema tests, SDK/CLI/TUI/MCP suites, and security tests |
| Package validation | Build, `twine check`, and wheel install/import smoke tests in CI |
| Documentation | README plus MkDocs site covering install, auth, requests, typed SDK, schema, errors, branching, mock API, CLI, TUI, and MCP |
...
- `mcp`: schema-driven MCP server with stdio and authenticated Streamable HTTP transports
