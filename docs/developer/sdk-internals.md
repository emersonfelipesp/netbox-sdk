...
A per-path index file can exist but fail to parse — truncated by a crash mid-write on a filesystem without atomic-rename guarantees, hand-edited, or corrupted by an out-of-band process. `_load_index_state()`, used by `save()`, `refresh()`, and `path_generation()`, degrades any unreadable or malformed index to a safe empty `(0, [])` state; that degradation is correct for those three callers because they only use the index to *register or fence* new writes, and a reset generation only costs a few extra cache misses for in-flight fenced saves, never a stale hit. `invalidate_path()` cannot share that same degradation: it purges entries by walking the keys the index claims to have registered, so treating a corrupted index as "no keys registered" silently skips deleting the entry files a prior, unreadable index actually did register — the exact stale-servable-forever failure this store exists to prevent, since `load()` decides hits by entry-file existence alone and never consults the index. `invalidate_path()` therefore uses a distinct loader, `_load_index_state_or_none()`, that returns `None` instead of `(0, [])` when the index file exists but cannot be parsed as valid JSON with the expected shape (still returning `(0, [])`, not `None`, when the file is simply absent — the ordinary case for a path never previously written). On `None`, `invalidate_path()` cannot know which entries the corrupted index registered, so it falls back to `_purge_all_entries()`, which deletes every `*.json` file under the cache root — every entry file and every other path's index file, not only the corrupted path's own — rather than attempting a narrower, unprovable purge. This is safe under the same generation-fencing invariant documented above: resetting every path's generation back to 0 only discards in-flight fence tokens, turning any concurrent fenced `save()`/`refresh()` into an extra cache miss, never a stale hit.
