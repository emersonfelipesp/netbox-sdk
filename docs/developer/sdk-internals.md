...
The per-path index's read-modify-write is serialized by `HttpCacheStore._locked_index()`. On platforms with `fcntl` (Linux, macOS), `_acquire_flock()` polls `flock(LOCK_EX | LOCK_NB)` on the same bounded `timeout`/`poll_interval` deadline `_portable_lock` already uses, raising `TimeoutError` if the deadline passes rather than calling the blocking `flock(LOCK_EX)` form. A blocking acquire here would freeze the entire caller — including the asyncio event loop, since `_locked_index()` is invoked synchronously from `NetBoxApiClient.request()` with no `run_in_executor`/`to_thread` wrapping — for as long as any other live, stopped, or wedged process held the lock, even after that process's own write had already committed against NetBox; every caller's existing lock-timeout handling (cache bypass, `_mark_path_unavailable`) now applies identically to both lock backends. Where `fcntl` is unavailable, the portable fallback (`_portable_lock`) uses `O_CREAT | O_EXCL` atomic file creation — guaranteed exclusive-create semantics on every supported filesystem — bounded by the same kind of timeout.
