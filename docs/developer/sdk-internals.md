...
Dot-segment resolution alone is not enough: aiohttp builds the actual outbound request via `yarl.URL(str, encoded=False)`, which percent-decodes *each* `/`-delimited path segment (without treating an encoded `%2f` as a separator) before resolving any segment whose decoded form is `.`/`..`. A percent-encoded alias such as `/api/dcim/%2e%2e/ipam/prefixes/5/` therefore also resolves to `/api/ipam/prefixes/5/` on the wire, even though neither raw segment is a literal dot that `posixpath.normpath()` alone would catch. `_normalize_request_path()` decodes each segment with `urllib.parse.unquote()` and substitutes only the segments whose decoded form is exactly `.`/`..` before the existing `posixpath.normpath()`-based resolution — mirroring yarl's actual per-segment algorithm rather than only its literal-dot subset. Without this, a write through a percent-encoded alias would mutate the canonical resource on the wire while invalidating cache entries keyed to the literal encoded text, leaving the canonical cached entry stale and servable by a verification read.
...
`_load_index_state_or_purge()` closes this by giving all four callers the same fail-safe response to a corrupted index: on `None` from `_load_index_state_or_none()`, it calls `_purge_all_entries()`, which deletes every `*.json` file under the cache root — every entry file and every other path's index file, not only the corrupted path's own — rather than attempting a narrower, unprovable purge, and returns a fresh `(0, [])` to the caller. This is safe under the same generation-fencing invariant documented above: resetting every path's generation back to 0 only discards in-flight fence tokens, turning any concurrent fenced `save()`/`refresh()` into an extra cache miss, never a stale hit — so it is safe to trigger this whole-store purge from any of the four call sites, whichever happens to be the first to encounter the corruption.

An earlier version of `_purge_all_entries()` deleted every `*.json` file without taking out any lock at all, on the (incomplete) reasoning that the generation-fencing invariant above already covered every hazard a whole-store purge could introduce. It missed one: `save()` and `refresh()` register a key in a path's index file *before* writing that key's entry file (see above), and an unlocked purge triggered by a *different* corrupted path could run its glob-and-unlink pass in the gap between those two writes — deleting the just-written index after the key was registered but before the entry file existed. The subsequent entry-file write then landed on disk unindexed: `load()` decides hits by entry-file existence alone, so it was servable as a permanently fresh hit that no later `invalidate_path()` for that path could ever discover and purge, since the index that would have listed it was already gone — the same failure mode the corrupted-index fix above closes, reopened through an unrelated path's write instead of a corrupted read.

`_purge_all_entries()` now takes out the same lock `save()`, `refresh()`, and `invalidate_path()` take around their own index-then-entry write pair — a nominal path, `_global_guard_path()`, passed through the existing per-path `_locked_index()` primitive rather than a second locking mechanism. This forces the two operations into a strict order: a concurrent writer for another path either finishes its whole index+entry pair before a purge can start (so the purge deletes both files together, leaving nothing orphaned) or only starts writing after the purge has already finished (so it writes onto an already-clean state). The lock is acquired only around each method's actual write section, never around the read-then-decide phase that discovers the corruption and calls `_purge_all_entries()` in the first place — holding it there would have the corrupted path's own call deadlock against itself when it reaches its own write phase.
