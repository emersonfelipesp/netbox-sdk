...
Um arquivo de índice por caminho pode existir mas falhar ao ser interpretado — truncado por uma falha em meio à gravação em um sistema de arquivos sem garantias atômicas de renomeação, editado manualmente, ou corrompido por um processo externo. `_load_index_state()`, usado por `save()`, `refresh()` e `path_generation()`, degrada qualquer índice ilegível ou malformado para um estado seguro e vazio `(0, [])`; essa degradação é correta para esses três chamadores porque eles só usam o índice para *registrar ou fazer fencing* de novas escritas, e uma geração reiniciada só custa alguns cache misses extras para `save()`s com fencing em andamento, nunca um acerto obsoleto. `invalidate_path()` não pode compartilhar essa mesma degradação: ele purga entradas percorrendo as chaves que o índice afirma ter registrado, então tratar um índice corrompido como "nenhuma chave registrada" ignora silenciosamente a exclusão dos arquivos de entrada que um índice anterior e ilegível de fato registrou — exatamente o modo de falha de "servível obsoleto para sempre" que este armazenamento existe para prevenir, já que `load()` decide acertos apenas pela existência do arquivo de entrada, nunca consultando o índice. `invalidate_path()` portanto usa um carregador distinto, `_load_index_state_or_none()`, que retorna `None` em vez de `(0, [])` quando o arquivo de índice existe mas não pode ser interpretado como JSON válido no formato esperado (ainda retornando `(0, [])`, não `None`, quando o arquivo simplesmente está ausente — o caso comum para um caminho nunca escrito antes). Ao receber `None`, `invalidate_path()` não pode saber quais entradas o índice corrompido registrou, então recorre a `_purge_all_entries()`, que exclui todo arquivo `*.json` sob a raiz do cache — todo arquivo de entrada e todo índice de outros caminhos, não apenas o do caminho corrompido — em vez de tentar uma purga mais restrita e improvável. Isso é seguro sob o mesmo invariante de fencing por geração documentado acima: reiniciar a geração de todo caminho para 0 só descarta tokens de fencing em andamento, transformando qualquer `save()`/`refresh()` concorrente com fencing em um cache miss extra, nunca em um acerto obsoleto.
