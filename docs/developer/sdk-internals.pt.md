...
Cabeçalhos de requisição se sobrepõem em três camadas, da menor para a maior precedência: `persistent_headers` (nível do cliente, ex.: o branch ativo da TUI ou o override de caller encaminhado pelo MCP), `_scoped_headers` (um `contextvars.ContextVar` por tarefa, definido por `header_scope()`/`activate_branch()`) e o argumento `headers` por chamada. Nomes de cabeçalho HTTP não diferenciam maiúsculas de minúsculas, mas um `dict` comum sim, então `_extract_case_insensitive()` remove toda variante de caixa de um nome de cabeçalho (`Authorization`, `authorization`, `AUTHORIZATION`, ...) de um dict de cabeçalhos e retorna o único valor correspondente mais o restante com todas as variantes removidas.

`_request_impl()` e `stream_sse()` chamam `_extract_case_insensitive(..., "Authorization")` em cada uma das três camadas **separadamente, antes de mesclá-las**, e então resolvem a precedência explicitamente: `call_authorization or scoped_authorization or persistent_authorization`. Essa ordem importa porque mesclar primeiro e depois extrair "o" valor de Authorization do dict combinado não é equivalente — o `update()` de um dict comum não reordena uma chave pré-existente quando um cabeçalho de mesmo nome mas caixa diferente de uma camada posterior e de maior precedência é aplicado; ele só anexa chaves genuinamente novas ao final. Mesclar persistente `{"Authorization": ...}` depois escopado `{"authorization": ...}` deixaria a chave escopada por último na ordem de iteração do dict combinado, então uma extração ingênua de "a última correspondência vence" a partir do dict mesclado deixaria o valor *escopado* vencer até mesmo sobre um `Authorization` por chamada de mesma caixa que deveria ter precedência sobre ambos — executando silenciosamente a requisição de um chamador sob a credencial de outro chamador. Extrair por camada e resolver a precedência em código contorna completamente a ordem de iteração do dict.
...
O `_ACTION_CROSS_RESOURCE_CACHE_PATHS` em nível de módulo em `client.py` mapeia o segmento final do caminho de cada ação mutante conhecida (`available-ips`, `available-prefixes`, `available-vlans`) para o(s) caminho(s) de coleção extra que ela realmente popula. `_trailing_action_name()` reconhece o formato de uma ação de detalhe — exatamente cinco segmentos de caminho não vazios (`api / app / resource / id / action`) com um `id` numérico — e `_related_cache_paths()` estende seu resultado com a(s) coleção(ões) mapeada(s) sempre que o caminho gravado corresponde. Ações somente leitura (`napalm`, `trace`, `units`, `elevation`, `paths`) são omitidas do mapeamento, já que um GET nunca torna nada obsoleto.
...
`_load_index_state_or_purge()` fecha essa brecha dando aos quatro chamadores a mesma resposta segura contra falhas para um índice corrompido: ao receber `None` de `_load_index_state_or_none()`, ele chama `_purge_all_entries(corrupted_index_path)` e retorna um `(0, [])` novo ao chamador.
