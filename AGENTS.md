...
- Private publisher `setup-uv` steps must set `github-token: ''`, require the reviewed uv archive SHA-256, and use a new per-run `RUNNER_TOOL_CACHE`; only the final sealed publish step may receive the separate package-write credential.
