...
- Private Gitea artifact handoffs pin `actions/upload-artifact` v3.1.3 and `actions/download-artifact` v3.0.2 by full SHA. This Gitea Actions runtime rejects v4+ because its artifact service is not implemented; do not advance the major without an explicit compatibility run.
