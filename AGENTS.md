...
`main`. Candidate build code runs without package credentials on
`ci-untrusted-python312`; the `mirror-host` publisher receives only the bounded
wheel/sdist/manifest transfer and independently binds both archives to verified
Git objects without importing or extracting candidate package code. Only its
final publish step receives the ephemeral job token. GitHub Actions remains the
sole TestPyPI and PyPI publisher.
...
- The private-registry publisher accepts only an absent version or the exact two-file wheel/sdist set with matching bytes and repository association. Partial, extra, mismatched, redirected, oversized, timed-out, or wrongly associated states fail closed; ambiguous upload/link responses recover only after an independent exact-state GET. Never move the job token into workflow/job scope or expose it to checkout, canonical ref validation, build, or artifact handling.
