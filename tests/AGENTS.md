...
- SDK-affecting branch/PR changes and every direct push to `main` run live NetBox SDK integration tests against `v4.7.0`, `v4.6.6`, `v4.6.3`, `v4.6.2`, and `v4.5.10`. Every source tag must resolve to its reviewed full commit; `v4.7.0` runs from its reviewed OCI digest, and lines without a reviewed digest run from verified source. Provenance-managed lines also bind the source path to the reviewed Git blob and normalized committed bundle. Each live job passes `NETBOX_EXPECTED_VERSION` from the matrix tag and then requires an exact `/api/status/` match.
