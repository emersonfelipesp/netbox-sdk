...
def _dist(tmp_path: Path) -> tuple[Path, Path, Path]:
    dist = tmp_path / "dist"
    dist.mkdir()
    wheel = dist / "netbox_sdk-0.0.11rc2-py3-none-any.whl"
    sdist = dist / "netbox_sdk-0.0.11rc2.tar.gz"
...
def test_prepare_upload_rejects_unexpected_remote_artifact(tmp_path: Path) -> None:
    dist, wheel, _ = _dist(tmp_path)
    unexpected = tmp_path / "netbox_sdk-0.0.11rc2-cp313-cp313-manylinux.whl"
...
@pytest.mark.parametrize(
    ...
    [
        "checksums.txt",
        "netbox_sdk-0.0.11rc2-py3-none-any.whl.sig",
    ],
)
...
def test_local_artifacts_reject_multiple_wheels(tmp_path: Path) -> None:
    dist, _, _ = _dist(tmp_path)
    (dist / "netbox_sdk-0.0.11rc2-1-py3-none-any.whl").write_bytes(b"extra")
...
@pytest.mark.parametrize(
    ...
    [
        ("wheel", "other_sdk-0.0.11rc2-py3-none-any.whl"),
        ...
        published_artifacts(
            {
                "urls": [
                    {
                        "filename": "netbox_sdk-0.0.11rc2-py3-none-any.whl",
                        "digests": {"sha256": "0" * 64},
