...
emits a private exact seal. A separate package-write job checks out its helper at
the same immutable source SHA and only validates the bounded seal, re-hashes the
two files, and performs bounded registry/Twine operations. Only its final step
