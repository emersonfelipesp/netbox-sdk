...
### Refreshing the Django model-build archive

The weekly `.github/workflows/django-model-builds.yml` workflow is read-only. It
builds portable graphs for the latest three NetBox releases and retains them as
a 14-day artifact; it never updates `django_models_builds/` or GitHub `main`.
Archive updates must enter through a reviewed Gitea pull request.

Download a selected workflow artifact into `.tmp/`, then run the local refresh
...
The refresh command performs no downloads. It normalizes the artifact into
`django_models_builds/`, rejects any absolute recorded path that cannot be made
relative to the NetBox checkout root, regenerates the bundled catalog, and
prints the exact `git status` for review. To build from an existing
local NetBox checkout instead, use:
...
A local-tag build is accepted only when `HEAD` resolves to the requested tag
commit and `git status --porcelain --untracked-files=all` is empty. This prevents
modified tracked files or untracked files from being archived under an official
release name.

The bundled catalog retains every previously packaged exact patch build.
`catalog.load_build("v4.6.3")` loads that exact artifact, while release-line
matching for `4.6` selects the newest numeric patch available in that line. The
manifest records line defaults in `builds` and the complete packaged inventory
in `exact_builds`. `scripts/build_model_catalog.py` is additive by default;
`--prune` is reserved for an explicit compatibility decision to remove older
exact builds.

Review the resulting archive and catalog changes, then open a Gitea pull
request. The offline suite fails with an actionable message when the archive
does not contain the SDK's newest stable release line.
...
`PACKAGE_WRITE_TOKEN` secret. The publish helper is given the public HTTPS origin
`https://git.nmulti.cloud`; the Gitea runner's `github.server_url` is an internal
HTTP address and is not a credential-free HTTPS origin. GitHub Actions remains the sole TestPyPI and PyPI
publisher.
...
- Generate **`metadata.json`** with `scripts/build_metadata.py`. The authoritative provenance field is `source.content_id`: a SHA-256 digest over the UTF-8 bytes of the sorted `git ls-tree -r --full-tree` lines for the candidate tree, with only the root `metadata.json` entry removed. Excluding `metadata.json` prevents the digest from containing itself; nested files named `metadata.json` remain covered, while empty subtrees do not appear in recursive `ls-tree` output and therefore do not affect the identity. Generation stages the current checkout in a temporary index and object database, so it does not modify the real index and the digest matches the eventual committed tree when all generated files are committed together. The exact schema rejects missing and unknown fields, pins `source.repo` to the trusted canonical repository identity, derives `python` and `netbox` from the same project sources used by generation, and requires `generated_at` to be a valid RFC 3339 UTC timestamp. `source.version` must match `project.version`. `source.commit` remains a full informational SHA and may identify the reviewed topic or canonical-main squash SHA; during generation and verification, an available object must be a commit with the same project version and content identity as the exact candidate tree, but an unavailable object does not invalidate the content identity. Content equality authenticates the tree, not the repository origin; an external trusted fetch or equivalent canonical-source binding must authenticate the origin. `scripts/build_metadata.py --verify` validates committed metadata against `HEAD`.
...
- A final GitHub Release must bind `--target` to the reviewed canonical-main SHA after `git ls-remote` shows the tag is absent. `--target` is ignored when the tag already exists. `scripts/release_policy.py` requires a `release` event tag commit to equal that freshly fetched main SHA, not merely be an ancestor.
