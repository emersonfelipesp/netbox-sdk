...
- Private publishers must download the pinned uv archive directly, verify the reviewed SHA-256 before extraction, use only the resulting absolute executable, and create a new per-run `UV_PYTHON_INSTALL_DIR`; only the final sealed publish step may receive the repository `PACKAGE_WRITE_TOKEN` credential. The Gitea Actions job token is not a package-registry credential.
