...
- Private Gitea artifact handoffs pin the maintained Node 20 backports `actions/upload-artifact` v3.2.2-node20 and `actions/download-artifact` v3.1.0-node20 by full SHA. This Gitea Actions runtime rejects v4+ because its artifact service is not implemented; `.gitea/workflows/artifact-v3-compatibility.yml` must pass across the untrusted and trusted runner labels on the release PR before a candidate tag, and the major must not advance without a new explicit compatibility run.
