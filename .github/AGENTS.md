...
- `workflows/test.yml`
  ...
  - adds the `mock` extra for mock API coverage and runs live NetBox tests for SDK-affecting branch/PR changes and every `main` push against `v4.7.0-beta2`, `v4.6.6`, `v4.6.3`, `v4.6.2`, and `v4.5.10`. Each live job binds `NETBOX_EXPECTED_VERSION` to the matrix tag and fails closed unless `/api/status/` matches `scripts/check_live_netbox_status.py`. Stable pins require an exact match (leading `v` stripped). The digest-pinned `v4.7.0-beta2` image accepts only `/api/status/` `4.7.0`. The digest pin has no mutable-tag fallback
